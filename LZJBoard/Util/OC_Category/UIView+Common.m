...
- (void)animationForSizeBounceWithAnimationName:(NSString *)name{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        
        POPSpringAnimation *anim = [POPSpringAnimation animationWithPropertyNamed:kPOPLayerScaleXY];
        anim.toValue = [NSValue valueWithCGPoint:CGPointMake(0.95, 0.95)];
        anim.springBounciness = 15.f;
        anim.repeatCount = 200;
        anim.autoreverses = YES;

        [self.layer pop_addAnimation:anim forKey:name];
        
    });
    NSLog(@"current pop animation-->%@",[self pop_animationKeys]);
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _monkeyView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"加载"]];
...
- (void)configWithType:(EaseBlankPageType)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void (^)(id))block{
    ...
    if (hasError) {
        ...
        switch (blankPageType) {
            ...
            case EaseBlankPageTypeCollect:
            {
                imageName = @"blank__xxxx_ooo";
                tipStr = @"木有收藏\n快去收藏宝贝吧～";
            }
                break;
            case EaseBlankPageTypeGoodsDetails:
            {
                imageName = @"blank__xxxx_ooo";
                tipStr = @"获取商品信息失败~\(≧▽≦)/~";
            }
            ...
            case EaseBlankPageTypeSysnotification:
            {
                imageName = @"blank__xxxx_ooo";
                tipStr = @"没有系统通知!";
            }
